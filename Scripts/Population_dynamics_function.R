                    ...
                    low.vs.high = 0.4,method="not_sample",mod.preds = NULL)
{
  ...
  if(method == 'bp_resample')
  {
    ...
    if(bm.start <= stock.pool) lam.samp <- stock.dat$lam.no.fish.ltr[samp] # Get the sample years.  
  ...
  if(method == "bp_log_normal")
  {
    ...
    if(bm.start < low.vs.high.bm) 
    {
      ...
      {
        lam.mn <- mean(stock.dat$lam.no.fish.ltr[low.bm.years],na.rm=T)
      ...
      if(length(low.bm.years) ==0)
      {
        lam.mn <- mean(stock.dat$lam.no.fish.ltr,na.rm=T)
  ...
  if(method == "log_linear")
  {
    ...
    if(is.null(mod.preds))
    {
        ...
        if(p.val < 0.2)
        {
          ...
          mod.preds$lambda <- exp(mean(log(stock.dat$lam.no.fish.ltr),na.rm=T)) 
      ...
      if(bm.start <=stock.pool)
      {
        ...
        lam.mn <- mod.preds$lambda[mod.preds$bm.prop == bm.prop]
        lam.sd <- mod.preds$sd[1]
        lam.samp <- rlnorm(1,log(lam.mn),lam.sd)

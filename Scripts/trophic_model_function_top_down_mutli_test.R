...
tl.4.5.prop.bm <- bm.best$bm.tl[bm.best$troph.cat =="High"][1:n.years]/(bm.best$bm.tl[bm.best$troph.cat =="Medium"][1:n.years]+bm.best$bm.tl[bm.best$troph.cat =="High"][1:n.years])# This is the correlation between 4 and 5
...
# beta parameters which I'll use for the multinomoial
beta.params <- NULL
for(ss in stock.eco)
{
  #browser()
  tmp <- bm.best[bm.best$Stock ==ss,]
  fit <- fitdist(tmp$prop.bm.stock.tl, "beta", method = "mle")
  beta.params[[ss]] <- data.frame(s1 = fit$estimate[1],s2 = fit$estimate[2],trophic.cat = tmp$troph.cat[1],Stock = ss)
  
}
beta.params <- do.call('rbind',beta.params)
...
for(i in 1:n.sims) 
{
 ...
 # This starts the time series at the last value of the time series, then moves it to the mean value, bam!!  This will be done for each of these arima sims.
  ...
  sim.tl.5.prop.bm[[i]] <-inv.logit(mn.tl.5.logit + 
                                      arima.sim(model =list(ar = c(tl.5.prop.bm.lag.1)),n = n.yrs.proj,
                                                n.start =1, start.innov = c(start.tl.5.diff/tl.5.prop.bm.lag.1), 
                                                innov = c(0,rnorm(n.yrs.proj-1,0,sd.tl.5.logit))))
  
  #NEW: ARIMA simulation on the transfer efficiencies between TLs
  sim.tl.3.to.4[[i]] <- inv.logit(mn.tl.3.to.4.logit + 
                                    arima.sim(model =list(ar = c(tl.3.to.4.lag.1)),n = n.yrs.proj,
                                              n.start =1, start.innov = c(start.tl.3.to.4.diff/tl.3.to.4.lag.1), 
                                              innov = c(0,rnorm(n.yrs.proj-1,0,sd.tl.3.to.4.logit))))
  ...
  bm.sim.5[[i]] <- sim.tl.5.prop.bm[[i]] * sim.com.bm[[i]]$bm
  bm.sim.4[[i]] <- bm.sim.5[[i]]/sim.tl.4.to.5[[i]]
  bm.sim.3[[i]] <- bm.sim.4[[i]]/sim.tl.3.to.4[[i]]
  
  #check
  #check <- data.frame(sim.com.bm$bm, bm.sim.3, bm.sim.4, bm.sim.5, bm.check=(bm.sim.3 + bm.sim.4 + bm.sim.5))

  
  #bm.sim.3 <- sim.tl.3.prop.bm * sim.com.bm[[i]]$bm
  # So this is what is left for 3 and 4
  #bm.left.4.5<- sim.com.bm[[i]]$bm - bm.sim.3
  ...
  for(tl in troph.levels)
  {
    ...
    for(b in tl.stocks) 
    {
      p.tmp <- beta.params[beta.params$trophic.cat == tl & beta.params$Stock==b,]
      prop.tmp[[b]] <- data.frame(prop =rbeta(n.yrs.proj,p.tmp$s1*10,p.tmp$s2*10),year = 1:n.yrs.proj,Stock = b)
    }
    prop.samp <- do.call('rbind',prop.tmp)
    if(tl == "Low") bm.samp <- bm.sim.3[[i]]
    if(tl == "Medium") bm.samp <- bm.sim.4[[i]]
    if(tl == "High") bm.samp <- bm.sim.5[[i]]
    
    tst <- NULL
    for(t in 1:n.yrs.proj)
    {
      dat.t <- prop.samp[prop.samp$year == t,]
      tst[[t]] <- data.frame(bm.stock = rmultinom(1,bm.samp[t],prob = dat.t$prop),prop.bm.stock = dat.t$prop,
                             Years=t,Stock = dat.t$Stock,sim = i,troph.cat= tl)
    }
    tmp.tl[[tl]] <- do.call("rbind",tst)
  }# end tl
  sim.pool.stock.lst[[i]] <- do.call('rbind',tmp.tl)
...
# Now get the quantile data ready
...
save_plot(filename = paste0(repo.loc,"/Figures/TD/Simulation_stock_K.png"),sim.stock.pool.plt,base_height = 10,base_width = 20)
...
save_plot(filename = paste0(repo.loc,"/Figures/TD/Simulation_trophic_K.png"),sim.tl.pool.plt,base_height = 10,base_width = 20)
...
save_plot(filename = paste0(repo.loc,"/Figures/TD/Simulation_eco_K.png"),sim.com.pool.plt,base_height = 8,base_width = 11)
...
save_plot(filename = paste0(repo.loc,"/Figures/TD/Simulation_quantile_stock_K.png"),sim.stock.pool.quant.plt,base_height = 10,base_width = 20)
...
save_plot(filename = paste0(repo.loc,"/Figures/TD/Simulation_quantile_TL_K.png"),sim.TL.pool.quant.plt,base_height = 10,base_width = 20)
...
save_plot(filename = paste0(repo.loc,"/Figures/TD/Simulation_quantile_eco_K.png"),sim.com.pool.quant.plt,base_height = 10,base_width = 20)
...
for(j in 1:n.sims)
{
  ...
  for(t in 1:n.yrs.proj)
  {
    ...
    for (tl in rev(troph.levels)) {
      if (tl == "High") tl.pool <- bm.sim.5[[j]][t]
      if (tl != "High") tl.pool.space.prop <- Ks[[j]]$pool.space[Ks[[j]]$tl==tl & Ks[[j]]$Years==t]
      ...
      if (tl == "High") {
        ...
        base.stock.pool.tmp$pool.space <- base.stock.pool.tmp$bm.stock * (base.tl.pool.tmp$prop.pool.space-1)
      ...
      for (s in tl.stocks) 
      {
        ...
        ## Now calculate the exploitation rate, which will be based on the management strategy for the stock
        #outlined above.
      ...
      # DK note Now what makes sense for dynamics here?? Idea for top down, we find the difference between the carrying capacity of the system 
      # and the biomass observed in the top trophic levels.  This K-space is 'given' to the
      # trophic level below.  If the stocks are above the carrying capacity of the top trophic level, then 
      # that is going to result in the lower trophic level having it's carrying capacity reduced (and of course vice versa)
      # This should cascade down through the system causing a negative feedback.
     
      if (tl == "High") 
      {
        
        bms <- do.call('rbind', bm.stash)
        # This way takes the proportion of k-space available (or missing) and gives it to the TL below, so if TL5 is at
        # 90% of their K-space, that means TL4 gets a 1/0.9 'bonus' in their K space next year
        # Doing it this way to ensure that the K's remain positive (by adding actual space you can get negatives....)
        next.tl.pool.space.prop <- 1/(sum(bms$bm.stock)/sum(bms$pool.init))#/mean(sim.tl.4.to.5[[j]]) # Old way this was used to give them the tl efficicnecy, worked badly...
      }
      if (tl == "Medium") 
      {
        bms <- do.call('rbind', bm.stash)
...
saveRDS(object = ts.final,file = paste0(repo.loc,"/Results/TD/NS_projections_",n.sims,"_sims_",min(years),"_to_",max(years),
                                        "_time_series_projections.Rds"))

saveRDS(object = quants,file = paste0(repo.loc,"/Results/TD/NS_projections_",n.sims,"_sims_",min(years),"_to_",max(years),
                                      "_time_series_quantiles.Rds"))
# 
# saveRDS(object = r.final,file = paste0(repo.loc,"/Results/TD/NS_projections_",n.sims,"_sims_",min(years),"_to_",max(years),
#                                        "_r_projections.Rds"))
...
save_plot(paste0(repo.loc,"/Figures/TD/raw_biomass_trends.png"),p.sims,base_height = 12,base_width = 24)
...
save_plot(paste0(repo.loc,"/Figures/TD/raw_removals_trends.png"),p.removals,base_height = 12,base_width = 24)
...
save_plot(paste0(repo.loc,"/Figures/TD/raw_exploitation_rates.png"),p.er,base_height = 12,base_width = 24)
...
save_plot(paste0(repo.loc,"/Figures/TD/Quantile_biomass_trends.png"),p.sims.quants,base_height = 8,base_width = 16)
...
p.sims.quants.by.stock <- ggplot(quants) + geom_line(aes(x=Years+max(bm.best$Year),y=med/1e3),linewidth=1.25,color='grey50') + 
  ...
  scale_x_continuous(name='',breaks = few.breaks)
#geom_ribbon(data=quants, aes(x=Years,ymax=U.50,ymin = L.50),alpha=0.5,fill='blue',color='blue') 
save_plot(paste0(repo.loc,"/Figures/TD/Quantile_biomass_trends_by_stock.png"),p.sims.quants.by.stock,base_height = 12,base_width = 24)
...
p.sims.rems.by.stock <- ggplot(quants) + geom_line(aes(x=Years+max(bm.best$Year),y=rem.med/1e3),linewidth=1.25,color='grey50') + 
  ...
  scale_x_continuous(name='',breaks = few.breaks)
#geom_ribbon(data=quants, aes(x=Years,ymax=U.50,ymin = L.50),alpha=0.5,fill='blue',color='blue') 
save_plot(paste0(repo.loc,"/Figures/TD/Quantile_removals_by_stock.png"),p.sims.rems.by.stock,base_height = 12,base_width = 24)
#
p.sims.er.by.stock <- ggplot(quants) + geom_line(aes(x=Years+max(bm.best$Year),y=er.med),linewidth=1.25,color='grey50') + 
  ...
  scale_x_continuous(name='',breaks = few.breaks)
#geom_ribbon(data=quants, aes(x=Years,ymax=U.50,ymin = L.50),alpha=0.5,fill='blue',color='blue') 
save_plot(paste0(repo.loc,"/Figures/TD/Quantile_er_by_stock.png"),p.sims.er.by.stock,base_height = 12,base_width = 24)
